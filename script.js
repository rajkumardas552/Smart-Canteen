...
// Permanent LocalStorage Student Session Persistence
function initStudentSessionPersistence() {
  const nameInput = document.getElementById('name') || document.getElementById('student_name');
  const rollInput = document.getElementById('roll_number');
  const phoneInput = document.getElementById('phone');

  if (nameInput && !nameInput.value && localStorage.getItem('sc_student_name')) {
    nameInput.value = localStorage.getItem('sc_student_name');
  }
  if (rollInput && !rollInput.value && localStorage.getItem('sc_student_roll')) {
    rollInput.value = localStorage.getItem('sc_student_roll');
  }
  if (phoneInput && !phoneInput.value && localStorage.getItem('sc_student_phone')) {
    phoneInput.value = localStorage.getItem('sc_student_phone');
  }

  document.querySelectorAll('form').forEach(form => {
    form.addEventListener('submit', () => {
      const currentName = document.getElementById('name') || document.getElementById('student_name');
      const currentRoll = document.getElementById('roll_number');
      const currentPhone = document.getElementById('phone');

      if (currentName && currentName.value.trim()) localStorage.setItem('sc_student_name', currentName.value.trim());
      if (currentRoll && currentRoll.value.trim()) localStorage.setItem('sc_student_roll', currentRoll.value.trim());
      if (currentPhone && currentPhone.value.trim()) localStorage.setItem('sc_student_phone', currentPhone.value.trim());
    });
  });
}
